...
def test_origin_policy_allows_configured_hosts_and_origins():
    policy = OriginPolicy.from_csv(
        "localhost,127.0.0.1,https://rtc.example.test:8443"
    )
...
def test_origin_policy_from_csv_trims_empty_values():
    policy = OriginPolicy.from_csv(" localhost, , https://127.0.0.1:8080 ")
...
async def test_dashboard_proxy_rejects_unlisted_origin(aiohttp_client):
    settings = Settings(
        dashboard_origin_allowlist="localhost,127.0.0.1",
